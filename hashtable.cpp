...
void* slow_aligned_realloc(void* ptr, size_t size, size_t oldsize, size_t align){
    /*WHY LINUX DOES NOT HAVE ALIGNED REALLOC?????????????????????????????????????????????*/
    /* go here -> https://wiki.sei.cmu.edu/confluence/pages/viewpage.action?pageId=87152255 if you think this function is shit*/
